...
function snapshot(){normalizeEdges();for(const k of Object.keys(TYPES))shadow[k]=new Map((DB?.[k]||[]).map(x=>[entityId(k,x),clean(x)]))}
...
const oldSave=save;
save=function(){normalizeEdges();let now=new Date().toISOString();for(const [k,type] of Object.entries(TYPES)){let arr=DB?.[k]||[],prev=shadow[k]||new Map(),seen=new Set();for(const x of arr){let id=entityId(k,x);seen.add(id);if(prev.get(id)!==clean(x))x.updatedAt=now}for(const id of prev.keys())if(!seen.has(id))queueDelete(type,id,now)}oldSave();snapshot();window.GKHCloud?.scheduleSync?.()};
